    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-10 animate-fadeInUp">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="text-left flex-1 space-y-4 animate-slideInLeft">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/20 text-blue-200 text-xs font-semibold uppercase tracking-wider backdrop-blur-md">
              <span className="w-2 h-2 rounded-full bg-blue-400 animate-ping" /> Live in Pune District
            </div>
          ...
          <div className="hidden md:flex relative animate-float">
            <div className="w-48 h-48 bg-white/10 border border-white/20 rounded-3xl backdrop-blur-xl flex items-center justify-center shadow-2xl">
              <Package className="w-24 h-24 text-blue-300 drop-shadow-2xl" />
            </div>
            <div className="absolute -bottom-6 -left-6 w-24 h-24 bg-emerald-500/20 border border-emerald-500/30 rounded-2xl backdrop-blur-xl flex items-center justify-center shadow-xl animate-float2">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {steps.map((step, i) => (
            <button key={i} onClick={step.action} className={`card-3d stagger-${i+1} group relative bg-white border border-slate-200 rounded-3xl p-6 text-left hover:border-blue-300 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300 w-full overflow-hidden`}>
      ...
      <div className="relative rounded-[2rem] bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200 p-8 md:p-10 overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {safetyItems.map((f, i) => (
              <div key={i} className={`card-3d stagger-${i+1} bg-white rounded-2xl p-6 shadow-sm border border-slate-100`}>
                <div className={`w-12 h-12 rounded-xl ${f.color} flex items-center justify-center mb-4 animate-float3`}>
                  {f.icon}
                </div>
