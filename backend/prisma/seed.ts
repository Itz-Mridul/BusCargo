...
async function main() {
  ...
  const cityData = [
    // Pilot corridor (Kopargaon is the primary pilot)
  ...
  for (const c of cityData) {
    ...
    cityMap[c.name] = city;
    const cbs = await prisma.depot.create({ data: { name: `${c.name} Central Bus Stand`, cityId: city.id, lat: c.lat, lng: c.lng } });
    depotMap[c.name] = cbs;
  }
  ...
  const shortAlias: Record<string, any> = {
    'Dadar': dadar, 'Andheri': andheri, 'Borivali': borivali, 'Kurla': kurla, 'Bandra': bandra,
    'Swargate': swargate, 'Katraj': katraj, 'Wakad': wakad, 'Hadapsar': hadapsar, 'Kothrud': kothrud,
    'Sitabuldi': sitabuldi, 'Kamptee': kamptee,
  };

  const createRoute = async (name: string, type: string, src: string, dst: string, stops: string[]) => {
    const waypoints = stops.map(s => { const d = shortAlias[s] ?? depotMap[s]; return { lat: d.lat, lng: d.lng }; });
    const route = await prisma.route.create({ data: { name, type, sourceCityId: cityMap[src].id, destCityId: cityMap[dst].id, waypointsJson: JSON.stringify(waypoints) } });
    for (let i = 0; i < stops.length; i++) {
      const depot = shortAlias[stops[i]] ?? depotMap[stops[i]];
    ...
    for (let b = 0; b < busCount; b++) {
      await prisma.bus.create({ data: { routeId: route.id, currentLat: waypoints[0].lat, currentLng: waypoints[0].lng, status: 'IDLE' } });
    }
  };

  // ─── PILOT CORRIDOR (Kopargaon - the primary demo route) ─────
  await createRoute('Kopargaon - Ahmednagar - Pune (Pilot MoU)',   'INTERCITY', 'Kopargaon', 'Pune',      ['Kopargaon', 'Sangamner', 'Ahmednagar', 'Pune']);
  await createRoute('Kopargaon - Shirdi Express',                  'INTERCITY', 'Kopargaon', 'Shirdi',    ['Kopargaon', 'Shirdi']);
  await createRoute('Kopargaon - Nashik MSRTC',                   'INTERCITY', 'Kopargaon', 'Nashik',    ['Kopargaon', 'Sangamner', 'Nashik']);
  await createRoute('Kopargaon - Aurangabad MSRTC',               'INTERCITY', 'Kopargaon', 'Aurangabad',['Kopargaon', 'Jalna', 'Aurangabad']);
  await createRoute('Kopargaon - Mumbai Express',                  'INTERCITY', 'Kopargaon', 'Mumbai',    ['Kopargaon', 'Sangamner', 'Nashik', 'Mumbai']);
  await createRoute('Kopargaon - Beed Express',                    'INTERCITY', 'Kopargaon', 'Beed',      ['Kopargaon', 'Beed']);
  await createRoute('Kopargaon - Baramati MSRTC',                  'INTERCITY', 'Kopargaon', 'Baramati',  ['Kopargaon', 'Ahmednagar', 'Baramati']);
  ...
  console.log('✅ Database seeded: Kopargaon pilot corridor + 35 cities + 30+ routes + specialty depots.');
}
